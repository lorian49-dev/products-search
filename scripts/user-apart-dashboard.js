document.addEventListener('DOMContentLoaded', () => {
    ...
    input_request.addEventListener('input', async (event) => {
        ...
        try {
            ...
            const respuesta_producto = await fetch(`../search-products.php?term=${textoDelUsuario}`);
    ...
    if (linkSell) {
        linkSell.addEventListener('click', () => {
            window.location.href = 'seller-apart-check.php'
        })
    }
    // Elementos del menú de categorías y ayuda
    // ------------------ BUSCADOR DE PRODUCTOS ------------------
    ...
    const contenidoModal = {
        ...
        0: () => `
            <h2>Editar tu información</h2>
            <form action="../CONTROLLERS/user-apart-dashboard-edit.php" method="POST">
                <label>Nombre:</label>
